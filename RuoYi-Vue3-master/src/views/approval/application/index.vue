    ...
    <el-dialog title="查看申请" v-model="viewDialogVisible" width="30%">
      <el-form :model="viewForm" label-width="100px">
        <el-form-item label="申请状态">
          <el-tag :type="getStatusType(viewForm.status)">{{ viewForm.status }}</el-tag>
        </el-form-item>
        <el-form-item label="企业描述">
          <el-input v-model="viewForm.enterpriseDescription" type="textarea" :rows="3" readonly></el-input>
        </el-form-item>
        <el-form-item label="审核意见" v-if="viewForm.status !== '待审批'">
          <el-input v-model="viewForm.reason" type="textarea" :rows="4" readonly></el-input>
        </el-form-item>
        <el-form-item label="审核意见" v-else>
          <el-input v-model="viewForm.reason" type="textarea" :rows="4" placeholder="请输入审核意见"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
      <span class="dialog-footer">
        <el-button @click="viewDialogVisible = false">关 闭</el-button>
        <el-button type="primary" @click="updateApplicationStatus" :loading="updateLoading" v-if="viewForm.status === '待审批'">确 定</el-button>
      </span>
      </template>
    </el-dialog>
...
function updateApplicationStatus() {
  ...
  ElMessageBox.confirm(`确认${actionName}该申请吗?`).then(function() {
    ...
    if (error.name === 'TimeoutError' || error.message.includes('timeout')) {
      ElMessage.error('系统接口超时，请检查邮箱授权码是否正常')
    } else {
